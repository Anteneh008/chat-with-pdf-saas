...
export const indexName = "wiredwisdm";
...
async function namespaceExists(
  ...
  const { namespaces } = await index.describeIndexStats(); // Retrieve index statistics, including namespaces.
  return namespaces?.[namespace] !== undefined; //Check if the namespace exists in the retrieved statistics and return a boolean
}
...
export async function generateEmbeddingsInPineconeVectoreStore(docId: string) {
  ...
  const embeddings = new OpenAIEmbeddings(); //Create an instance of OpenAIEmbeddings.

  const index = await pineconeClient.index(indexName); //Retrieve the Pinecone index using the pineconeClient
...
const generateLangchainCompletion = async (docId: string, question: string) => {
  ...
  // create a retriever to search through the vectore store
  ...
  const historyAwarePrompt = ChatPromptTemplate.fromMessages([
    ...
    [
      "user",
      "Given the above conversation, generate a search query to look up in order to get information relevant to the coneversation",
    ],
  ]);

  // Create a history-aware retriever chain that uses the model, retriever, and prompt
  console.log("--- Creating a history-aware retriever cahin... ---");
  ...
  console.log("--- Defining a prompt template for answering qustions... ");
